...
Ksig=lambda s,r: torch.exp(-r/(2*s*s)); sig_pair=d/np.sqrt(2*np.log(1/EPS))
@torch.no_grad()
...
def acc(sig,bs=2000):
    ...
    return c/n_te
nf=neff_far(sig_pair); A_mean=float(nf.mean()); A_p99=float(np.percentile(nf,99)); A_max=float(nf.max())
out={"name":NAME,"eps":EPS,"A_mean":A_mean,"A_p99":A_p99,"A_max":A_max}
for tag,A in [("meanA",A_mean),("p99A",A_p99),("maxA",A_max)]:
    sig=d/np.sqrt(2*np.log(A/EPS)); mx,fo=tailmax(sig)
    out[tag]={"A":A,"sigma":float(sig),"tail_max":mx,"frac_over_1":fo,"acc":acc(sig)}
    print(f"[{NAME} {tag}] A={A:.1f} sig={sig:.4f} tail_max={mx:.3f} over1={fo:.4f} acc={out[tag]['acc']*100:.2f}")
json.dump(out,open(f"maxacert_{NAME}.json","w"),indent=2)
print(f"WROTE maxacert_{NAME}.json {time.time()-t0:.1f}s")
