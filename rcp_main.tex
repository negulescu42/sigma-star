...
{\normalfont\small $^{1}$The Informational Buildup Foundation (IBF), Bucharest, Romania.\quad $^{2}$Bucharest, Romania.\quad $^{3}$Simion Stoilow Institute of Mathematics of the Romanian Academy (IMAR), Bucharest, Romania.} \\[0.2em]
{\normalfont\small Corresponding author: R. Negulescu (\texttt{radu@ibf.ro}). C. Bereanu: \texttt{c.bereanu@imar.ro}; M. T\u{a}nase: \texttt{mihai@gmail.com}.}}
\date{}
...
\caption{\textbf{Locality is trainable (HotpotQA).} Fine-tuning the same reader with a far-mass penalty $\lambda\,\bar m_\mathcal{F}$ (backbone frozen, three seeds; bands are seed standard deviations). (a) Analytic certified coverage rises from $4\%$ at the matched $\lambda{=}0$ control to $62\%$, and empirical compliance $\Pr(m_\mathcal{F}\!\le\!\varepsilon)$ from $14\%$ to $83\%$, while answer exact-match falls only from $0.407$ to $0.389$. (b) Mean outside-evidence mass falls from $0.41$ below the $\varepsilon=0.05$ budget. Zero bound violations at every $\lambda$.}
\label{fig:certtrain}
\end{figure}
