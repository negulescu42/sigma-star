...
def prep(s):
    ...
    return s
os.makedirs("eqimg_si",exist_ok=True)
EQ_DPI=300
...
for i,e in enumerate(eqs):
    ...
    fig.savefig(f"eqimg_si/eq{i}.png",dpi=EQ_DPI,bbox_inches="tight",pad_inches=0.05,facecolor=PARCH); plt.close(fig)
...
_base=72.0/EQ_DPI
_maxw=max(PILImage.open(f"eqimg_si/eq{i}.png").size[0] for i in range(len(eqs))) if eqs else 1
...
def header_footer(canvas,doc):
    ...
    canvas.drawString(MARGIN,10*mm,"The Resolution Calibration Principle \u2014 Supplementary Information")
...
def _bannerlist():
    ...
    return [Spacer(1,2),_meta_glyph(),Spacer(1,4),
            ...
            Paragraph(_smallcaps("Supplementary Information",9),ParagraphStyle("silabel",parent=title_st,fontName="Times-Bold",fontSize=9,textColor=colors.HexColor(SEPIA),tracking=2.0,spaceBefore=3,spaceAfter=2)),
...
banner=_bannerlist()
doc=BaseDocTemplate("rcp_si.pdf",pagesize=A4,leftMargin=MARGIN,rightMargin=MARGIN,topMargin=MARGIN,bottomMargin=MARGIN)
...
bts=open("rcp_si.pdf","rb").read()
pdf=pdfium.PdfDocument("rcp_si.pdf"); npg=len(pdf)
for idx in range(npg): pdf[idx].render(scale=1.5).to_pil().save(f"sipg{idx}.png")
pdf.close()
